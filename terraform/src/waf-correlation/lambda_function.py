...
def native_to_dynamodb(value: Any) -> Any:
    """Convert Python floats only at the DynamoDB write boundary."""
...
def save_finding(
    ...
    findings_table.put_item(Item=native_to_dynamodb(item))
...
def publish_finding_event(
    ...
    """Publish the routing event required by Lab 12a."""
    ...
    if response.get("FailedEntryCount", 0) != 0:
        raise RuntimeError(
            "EventBridge rejected the WAF finding event: "
